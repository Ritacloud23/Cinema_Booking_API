...
> A chronological record of the major technical problems encountered while building the ScreenHive backend, including their causes, solutions, results, and lessons learned.
...
Swagger's OAuth2 password flow sends credentials as form data, while the endpoint was initially expecting JSON.
...
We initially had only one PostgreSQL database:
...
The test database was then populated with the same tables used by ScreenHive:
...
### CI Follow-up Problem

Tests passed locally but failed in GitHub Actions because `screenhive_test` did not exist in the fresh CI PostgreSQL environment.

### CI Cause

The test database had been created manually in the local Docker PostgreSQL instance, but GitHub Actions creates a fresh PostgreSQL instance for every run.

### CI Solution

Added a CI step that creates `screenhive_test` before running `pytest`.

### Lesson

Local infrastructure changes do not exist in CI. Test environments must explicitly create all required resources.
...
A state transition must update every related piece of state. Expiring a hold without releasing the associated seat leaves the system inconsistent.

---

## 12. Tests Failed After Adding `Film.base_price`
...
## 13. Firestore Environment Variable Configuration Error
...
## 14. Firestore Live Board and SSE Integration
...
PostgreSQL is the source of truth for seat state, while Firestore was introduced for stream-shaped live data.

However, there was initially no connection between seat state changes in PostgreSQL and the Firestore live board or SSE stream.
...
PostgreSQL remains the authoritative source for seat state, while Firestore is used only for the live, stream-shaped representation. SSE then provides a persistent connection through which clients can receive those updates without repeatedly polling the API.

---

## 15. Firestore Initialization and CI Test Failures
...
## 16. Payment Success Did Not Invalidate Seat Map Cache
...
The problems above reinforced several important engineering principles used throughout ScreenHive.

## Database as the Source of Truth

PostgreSQL is authoritative for core application state, especially seat availability, holds, bookings, and payments.

## Transactions and Locking for Concurrency

Critical seat operations must be protected by database transactions and row-level locking rather than relying on application timing.

## Cache as a Projection

Redis improves read performance but must never become the source of truth. Cache invalidation must happen after successful database state changes.

## External Services Need Clear Boundaries

Firestore provides stream-shaped live data and SSE delivers those updates to clients. The core application must still work safely when Firestore is disabled in environments such as CI.

## Tests Must Reflect the Real Schema

When models change, test fixtures and seed data must change with them.

## CI Must Recreate Its Own Environment

Anything created manually on a developer's machine must also be explicitly created or configured in CI.

## Business State Must Follow the Real Workflow

A booking being created does not mean payment succeeded. ScreenHive therefore separates:
...
from the later confirmed state.

## Cache and Live Data Must Follow Database Changes

Whenever PostgreSQL state changes, dependent projections such as Redis seat maps and Firestore live boards must be updated or invalidated deliberately.

---

# Current Status

The documented problems have been resolved and converted into tests or implementation safeguards where appropriate.

The ScreenHive backend now includes:
...
- Row-level locking for seat holds
- Dedicated test database
- Redis seat-map caching
- Cache invalidation on seat-state changes
- Firestore live boards
- Server-Sent Events
...
This problem log should continue to be updated whenever a meaningful technical problem is solved during the remaining ScreenHive work.